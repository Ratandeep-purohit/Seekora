...
export default function ResultsPage() {
    ...
    return (
        <div className="min-h-screen relative flex flex-col text-[var(--color-text-primary)] overflow-hidden font-sans">
            ...
            {/* ===== MODERN HEADER ===== */}
            <header className="sticky top-0 z-50 glass-panel border-b border-[var(--color-border)] backdrop-blur-xl">
                <div className="flex items-center gap-6 px-6 lg:px-8 py-4">
                    ...
                    >
                        ...
                        <span className="font-display font-semibold text-xl tracking-wide text-[var(--color-text-primary)] group-hover:text-primary-100 transition-colors">Seekora</span>
                    </button>

                    {/* Search Bar */}
                    <form onSubmit={handleSearch} className="flex-1 max-w-[720px]">
                        <div className="relative flex items-center h-12 glass-panel border border-[var(--color-border)] rounded-2xl px-5 focus-within:border-primary-500/50 focus-within:shadow-[0_0_20px_rgba(99,102,241,0.2)] transition-all bg-[var(--color-surface)]">
                            ...
                            <input
                                ...
                                className="flex-1 bg-transparent text-[16px] text-[var(--color-text-primary)] outline-none font-light placeholder:text-[var(--color-text-tertiary)]"
                            ...
                            {localQuery && (
                                <button
                                    ...
                                    className="p-1 hover:bg-[var(--color-surface-hover)] rounded-full mr-2"
                                >
                                    <X className="w-4 h-4 text-[var(--color-text-secondary)]" />
                                </button>
                            )}
                            <div className="flex items-center gap-1 pl-3 border-l border-[var(--color-border)]">
                                <button type="button" className="p-2 hover:bg-[var(--color-surface-hover)] rounded-xl" title="Voice search">
                                    <Mic className="w-4 h-4 text-[var(--color-text-secondary)] hover:text-primary-400" />
                                </button>
                                <button type="button" className="p-2 hover:bg-[var(--color-surface-hover)] rounded-xl" title="Image search">
                                    <Camera className="w-4 h-4 text-[var(--color-text-secondary)] hover:text-primary-400" />
                                </button>
                            </div>
                        </div>
                    </form>
                    ...
                    <div className="hidden md:flex items-center gap-4 ml-auto shrink-0">
                        <ThemeSwitcher />
                        <button className="w-9 h-9 rounded-full bg-gradient-to-tr from-primary-500 to-primary-600 flex items-center justify-center text-[var(--color-text-primary)] shadow-lg shadow-primary-500/30 hover:shadow-primary-500/50 transition-all font-semibold border border-[var(--color-border)]">
                            S
                        </button>
                    </div>
                </div>
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-3 text-[14px] font-medium transition-all relative
                                    ${isActive ? 'text-primary-400' : 'text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]'}`}
                            >
            ...
            <main className="flex-1 relative z-10 mx-auto w-full max-w-[1400px]">
                ...
                {results?.meta && !isLoading && (
                    ...
                    >
                        <p className="text-[13px] text-[var(--color-text-secondary)] font-light flex items-center gap-2">
                ...
                {isLoading ? (
                    ...
                    <div className="max-w-[720px] ml-[20px] md:ml-[170px] lg:ml-[220px] mt-8">
                        <div className="glass-card p-8 text-center rounded-2xl border-red-500/20 bg-red-500/5">
                            ...
                            <p className="text-[var(--color-text-secondary)] text-sm">Failed to establish connection with the indexing core. Ensure the backend matrix is online.</p>
                        </div>
                    </div>
                    ...
                    <div className="max-w-[720px] ml-[20px] md:ml-[170px] lg:ml-[220px] mt-8 px-4 lg:px-0">
                        <div className="glass-card p-8 rounded-2xl">
                            <p className="text-[var(--color-text-primary)] text-lg font-light mb-4">
                                No data nodes match query <span className="text-primary-400 font-medium">"{query}"</span>
                            </p>
                            <p className="text-[var(--color-text-secondary)] text-sm font-medium mb-3 uppercase tracking-wider">Troubleshooting Protocol:</p>
                            <ul className="text-[var(--color-text-secondary)] text-sm space-y-2 list-none pl-0">
            ...
            {/* Minimal Footer */}
            <footer className="mt-auto py-6 border-t border-[var(--color-border)] relative z-10 bg-black/20 backdrop-blur-md">
                <div className="max-w-[1400px] mx-auto px-6 flex flex-col sm:flex-row items-center justify-between text-xs text-[var(--color-text-tertiary)] font-medium tracking-wide">
                ...
                {currentPage === 1 && results?.news?.length > 0 && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h3 className="text-lg text-[var(--color-text-primary)] font-medium">Real-time Intel</h3>
                        </div>
                        <div className="glass-card rounded-2xl overflow-hidden divide-y divide-white/5">
                            {results.news.slice(0, 3).map((news: any, i: number) => (
                                <a key={i} href={news.url} target="_blank" rel="noopener noreferrer" className="flex gap-4 p-5 hover:bg-[var(--color-surface)] transition-colors group">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-2 text-xs text-[var(--color-text-secondary)] font-medium">
                                            <span className="px-2 py-0.5 rounded text-primary-300 bg-primary-500/10 border border-primary-500/20">{news.source}</span>
                                            <span>• {news.time}</span>
                                        </div>
                                        <h4 className="text-base text-[var(--color-text-primary)] font-medium leading-snug line-clamp-2 group-hover:text-primary-300 transition-colors mb-2">
                                            {news.title}
                                        </h4>
                                    </div>
                                    {news.thumbnail && (
                                        <div className="w-28 h-20 rounded-xl overflow-hidden shrink-0 border border-[var(--color-border)] group-hover:border-primary-500/30 transition-colors">
                ...
                <div className="space-y-8">
                    {currentData.map((result: any, i: number) => (
                        <motion.article
                            ...
                            className="group glass-card p-5 rounded-2xl hover:bg-[var(--color-surface)] transition-colors border-transparent hover:border-[var(--color-border)]"
                        >
                            {/* Meta Track */}
                            <div className="flex items-center gap-3 mb-2.5">
                                <div className="w-7 h-7 rounded-full bg-[var(--color-surface)] flex items-center justify-center border border-[var(--color-border)] shrink-0 overflow-hidden">
                                    {result.favicon ? (
                                ...
                                <div className="min-w-0 flex flex-col justify-center">
                                    <span className="text-sm text-[var(--color-text-secondary)] truncate font-medium block leading-tight">
                                        {result.displayUrl || new URL(result.url).hostname.replace('www.', '')}
                                    </span>
                                    <span className="text-[11px] text-[var(--color-text-tertiary)] truncate block font-mono">
                                        {result.url}
                                    </span>
                                </div>
                            </div>
                            ...
                            {/* Data Snippet */}
                            <p className="text-[15px] text-[var(--color-text-secondary)] leading-relaxed line-clamp-3 font-light">
                                {result.snippet}
                            </p>

                            {/* Sitelinks - Mockup for depth */}
                            {i === 0 && result.source === 'google' && (
                                <div className="flex flex-wrap gap-x-6 gap-y-3 mt-4 pt-4 border-t border-[var(--color-border)]">
            ...
            <aside className="w-full xl:w-[400px] shrink-0 xl:pt-4">
                ...
                <div className="mt-8 glass-card rounded-2xl p-6">
                    <div className="flex items-center gap-2 mb-5">
                        ...
                        <h4 className="text-sm text-[var(--color-text-primary)] font-medium uppercase tracking-wider">Related Vectors</h4>
                    </div>
                    <div className="space-y-2">
                        {[
                            ...
                            <button
                                ...
                                className="flex items-center gap-3 w-full text-left bg-[var(--color-surface)] hover:bg-[var(--color-surface-hover)] px-4 py-3 rounded-xl transition-all border border-transparent hover:border-[var(--color-border)] group"
                            >
                                <Search className="w-4 h-4 text-[var(--color-text-tertiary)] group-hover:text-primary-400 transition-colors shrink-0" />
                                <span className="text-sm text-[var(--color-text-secondary)] group-hover:text-[var(--color-text-primary)] transition-colors">{item}</span>
                            </button>
                        ))}
                    </div>
                </div>
        ...
        <div className="px-6 lg:px-8 py-6 max-w-[1600px] mx-auto z-10 relative">
            <div className="columns-2 md:columns-3 lg:columns-4 xl:columns-5 gap-4">
                {currentData.map((img: any, i: number) => (
                    ...
                    >
                        <div className="rounded-2xl overflow-hidden glass-panel border-[var(--color-border)] relative bg-[var(--color-surface)]">
                            ...
                            <div className="absolute bottom-0 left-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20 translate-y-2 group-hover:translate-y-0">
                                <p className="text-sm text-[var(--color-text-primary)] line-clamp-2 font-medium mb-1 drop-shadow-md">{img.alt_text || 'Visual Asset'}</p>
                ...
                {selectedImage && (
                    ...
                    >
                        ...
                        >
                            ...
                            {/* Metadata Panel */}
                            <div className="w-full md:w-[360px] bg-[var(--color-surface)] border-l border-[var(--color-border)] p-6 flex flex-col">
                                <div className="flex items-start justify-between mb-6">
                                    <div>
                                        <h3 className="text-lg text-[var(--color-text-primary)] font-medium mb-2">{selectedImage.alt_text || 'Image Details'}</h3>
                                    ...
                                    <button
                                        onClick={() => setSelectedImage(null)}
                                        className="p-2 hover:bg-[var(--color-surface-hover)] rounded-full transition-colors bg-[var(--color-surface)]"
                                    >
                                ...
                                <div className="mt-auto pt-6 border-t border-[var(--color-border)]">
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="w-full py-3 bg-primary-600 hover:bg-primary-500 text-[var(--color-text-primary)] text-sm font-medium rounded-xl transition-colors flex items-center justify-center gap-2 shadow-lg shadow-primary-600/20"
                                    >
        ...
        <div className="max-w-[800px] ml-[20px] md:ml-[170px] lg:ml-[220px] pt-6 pb-12 px-4 lg:px-0 relative z-10">
            {currentData.map((vid: any, i: number) => (
                <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: i * 0.05 }}
                    className="mb-6 group glass-card p-4 rounded-2xl hover:bg-[var(--color-surface)] transition-all"
                >
                    <a href={vid.url} target="_blank" rel="noopener noreferrer" className="flex flex-col sm:flex-row gap-5">
                        {/* Futuristic Thumbnail */}
                        <div className="relative w-full sm:w-[280px] aspect-video rounded-xl overflow-hidden bg-black flex-shrink-0 border border-[var(--color-border)] group-hover:border-primary-500/50 transition-colors">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                                <div className="w-14 h-14 rounded-full bg-[var(--color-surface-hover)] backdrop-blur-md border border-[var(--color-border-hover)] flex items-center justify-center group-hover:bg-primary-500/50 group-hover:border-primary-400 group-hover:scale-110 transition-all duration-300 shadow-xl">
                                    <PlayCircle className="w-7 h-7 text-[var(--color-text-primary)] fill-white/80 group-hover:fill-white" />
                                </div>
                            </div>
                            <div className="absolute bottom-2 right-2 px-2 py-1 bg-black/60 backdrop-blur-md text-[var(--color-text-primary)] text-[11px] font-mono font-medium rounded-md border border-[var(--color-border)]">
                        ...
                        <div className="flex-1 min-w-0 py-1">
                            ...
                            <h3 className="text-lg text-[var(--color-text-primary)] font-medium leading-snug line-clamp-2 mb-2 group-hover:text-primary-300 transition-colors">
                                {vid.title}
                            </h3>
                            <p className="text-sm text-[var(--color-text-secondary)] line-clamp-2 font-light">
        ...
        <div className="max-w-[720px] ml-[20px] md:ml-[170px] lg:ml-[220px] pt-6 pb-12 px-4 lg:px-0 relative z-10">
            {currentData.map((news: any, i: number) => (
                <motion.div
                    ...
                    className="mb-6"
                >
                    <a href={news.url} target="_blank" rel="noopener noreferrer" className="flex flex-col sm:flex-row gap-5 group glass-card p-5 rounded-2xl hover:bg-[var(--color-surface)] transition-all">
                        {/* Main Content */}
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3 mb-2.5">
                                ...
                                <span className="text-[var(--color-text-tertiary)] text-xs font-mono border-l border-[var(--color-border)] pl-3">
                                    {news.time}
                                </span>
                            </div>
                            ...
                            <p className="text-[15px] text-[var(--color-text-secondary)] line-clamp-2 font-light leading-relaxed">
                                {news.snippet}
                            </p>
                        </div>
                        ...
                        {/* Thumbnail */}
                        {news.thumbnail && (
                            <div className="w-full sm:w-[160px] h-[120px] rounded-xl overflow-hidden bg-[var(--color-surface)] shrink-0 border border-[var(--color-border)] group-hover:border-primary-500/30 transition-colors">
                                <img src={news.thumbnail} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            </div>
                        )}
                    </a>
    ...
    return (
        ...
        >
            ...
            <div className="p-6 relative">
                {/* Float tag */}
                <span className="absolute top-[-14px] right-6 bg-primary-500 text-[var(--color-text-primary)] text-[10px] uppercase font-bold tracking-widest px-3 py-1 rounded-full shadow-lg">
                    Verified Schema
                </span>

                <h2 className="text-3xl font-display font-semibold text-[var(--color-text-primary)] mb-2 tracking-tight">
                    {data.title || query}
                </h2>
                ...
                <p className="text-[15px] text-[var(--color-text-secondary)] leading-relaxed font-light mb-5">
                    {data.description}
                </p>

                {data.url && (
                    <a href={data.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center w-full py-2.5 bg-[var(--color-surface)] hover:bg-[var(--color-surface-hover)] border border-[var(--color-border)] rounded-xl text-sm font-medium text-[var(--color-text-primary)] transition-all shadow-md">
                        Explore Full Wikipedia Archive <ExternalLink className="w-4 h-4 ml-2 text-[var(--color-text-secondary)]" />
                    </a>
                )}
            </div>

            <div className="border-t border-[var(--color-border)] bg-white/[0.02] px-6 py-4">
                <p className="text-xs text-[var(--color-text-tertiary)] text-center font-medium tracking-wide">
...
function GooglePagination({ currentPage, totalPages, onPageChange }: any) {
    ...
    return (
        <div className="flex flex-col items-center py-10 mt-8 border-t border-[var(--color-border)] relative z-10 w-full max-w-[720px] ml-[20px] md:ml-[170px] lg:ml-[220px]">
            <div className="flex items-center gap-2">
                {currentPage > 1 && (
                    <button
                        onClick={() => onPageChange(currentPage - 1)}
                        className="glass-panel w-10 h-10 flex items-center justify-center rounded-full hover:bg-[var(--color-surface-hover)] hover:text-primary-400 transition-colors mr-2"
                    >
                ...
                {Array.from({ length: maxVisible }, (_, i) => i + 1).map((p) => (
                    <button
                        ...
                        className={`w-10 h-10 flex items-center justify-center text-[14px] transition-all rounded-full font-medium
                            ${currentPage === p
                                ? 'bg-primary-500 text-[var(--color-text-primary)] shadow-lg shadow-primary-500/30 border border-primary-400'
                                : 'glass-panel text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-surface-hover)]'
                            }`}
                    >
                        {p}
                    </button>
                ...
                {currentPage < totalPages && (
                    <button
                        onClick={() => onPageChange(currentPage + 1)}
                        className="glass-panel w-10 h-10 flex items-center justify-center rounded-full hover:bg-[var(--color-surface-hover)] hover:text-primary-400 transition-colors ml-2"
                    >
...
function LoadingSkeleton() {
    return (
        <div className="max-w-[720px] ml-[20px] md:ml-[170px] lg:ml-[220px] pt-8 space-y-6 px-4 lg:px-0 relative z-10">
            {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="glass-card p-5 rounded-2xl border border-[var(--color-border)] skeleton-shimmer bg-white/[0.02]">
                    <div className="flex items-center gap-3 mb-3">
                        <div className="w-7 h-7 rounded-full bg-[var(--color-surface-hover)]" />
                        <div className="h-4 w-32 bg-[var(--color-surface-hover)] rounded" />
                    </div>
                    <div className="h-6 w-[80%] bg-[var(--color-surface-hover)] rounded mb-3" />
